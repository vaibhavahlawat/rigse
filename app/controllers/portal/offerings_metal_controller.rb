...
class Portal::OfferingsMetalController < ActionController::Metal
  ...
  def launch_status
    if (offering = Portal::Offering.find(params[:id])) && (current_user_or_guest = (session[:user_id] ? User.find(session[:user_id]) : nil)) && current_user_or_guest.portal_student
      learner = Portal::Learner.find_by_offering_id_and_student_id(offering.id, current_user_or_guest.portal_student.id)
      status_event_info = {}
