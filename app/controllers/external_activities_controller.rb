class ExternalActivitiesController < ApplicationController
  ...
  def can_edit
    if defined? @external_activity
      unless @external_activity.changeable?(current_user_or_guest)
        error_message = "you (#{current_user_or_guest.login}) can not #{action_name.humanize} #{@external_activity.name}"
  ...
  def index
    ...
    if params[:mine_only]
      @external_activities = @external_activities.reject { |i| i.user.id != current_user_or_guest.id }
    end
  ...
  def new
    @external_activity = ExternalActivity.new
    @external_activity.user = current_user_or_guest
    respond_to do |format|
  ...
  def duplicate
    ...
    (@external_activity.teacher_notes + @external_activity.author_notes).each {|tn| n.user = current_user_or_guest; n.save }
