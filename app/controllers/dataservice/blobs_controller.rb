class Dataservice::BlobsController < ApplicationController
  ...
  def is_admin?
    return (current_user_or_guest != nil && current_user_or_guest.has_role?('admin'))
  end
  ...
  def show
    ...
    is_authorized = is_admin? || (@dataservice_blob && @dataservice_blob.token == params[:token]) || current_user_or_guest.has_role?('researcher')
