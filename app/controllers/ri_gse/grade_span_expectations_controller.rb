  ...
  def index
    ...
    respond_to do |format|
      format.html do
        ...
        if params[:mine_only]
          @grade_span_expectations = RiGse::GradeSpanExpectation.search(params[:search], params[:page], self.current_user_or_guest, [{:expectations => [:expectation_indicators, :expectation_stem]}])
        else
