class ResourcePagesController < ApplicationController
  ...
  def show
    if current_user_or_guest.has_role? 'admin'
      ...
      @resource_page = ResourcePage.visible_to_user_with_drafts(current_user_or_guest).find(params[:id])
      # If this is a student, increment the counter on StudentViews
      if current_user_or_guest.portal_student
        @student_view = StudentView.find_or_create_by_user_id_and_viewable_id_and_viewable_type(current_user_or_guest.id,
                                                                                                @resource_page.id,
  ...
  def new
    @resource_page = current_user_or_guest.resource_pages.new
  end

  def create
    @resource_page = current_user_or_guest.resource_pages.new(params[:resource_page])
...
protected
  ...
  def teacher_required
    return if logged_in? && (current_user_or_guest.portal_teacher || current_user_or_guest.has_role?("admin"))
  ...
  def find_resource_page_and_verify_owner
    @resource_page = ResourcePage.find(params[:id])
    return if @resource_page.changeable?(current_user_or_guest)
    flash[:error] = "You're not authorized to do this"
    redirect_to :home
  end
end
