class TeacherNotesController < ApplicationController
  ...
  def set_owner(note)
    if (! note.authored_entity.nil?)
      ...
      note.user = current_user_or_guest
    end
  end
  ...
  def show_teacher_note
    if @teacher_note.changeable?(current_user_or_guest)
  ...
  def create
    if (@teacher_note.changeable?(current_user_or_guest) && @teacher_note.update_attributes(params[:teacher_note]))
      if (request.xhr?)
