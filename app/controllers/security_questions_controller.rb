class SecurityQuestionsController < ApplicationController
  ...
  # GET
  def edit
    @security_questions = SecurityQuestion.fill_array(current_user_or_guest_or_guest.security_questions)
  end

  # PUT
  def update
    @security_questions = SecurityQuestion.make_questions_from_hash_and_user(params[:security_questions], current_user_or_guest)
    errors = SecurityQuestion.errors_for_questions_list!(@security_questions)  
    if (!errors) || errors.empty?
      current_user_or_guest.update_security_questions!(@security_questions)
  ...
  def user_has_security_questions
    unless current_user_or_guest && !current_user_or_guest.portal_student.nil?
      redirect_to(root_path)
      return
    end
  end
  ...
  #     next if data.nil?
  #     
  #     existing_object = current_user_or_guest.security_questions.find_by_id(data[:id]) if data[:id] && data[:question_idx] == "current"
  #     
