class AuthorNotesController < ApplicationController
  ...
  def setup_object
    if params[:id]
      ...
      @author_note.user = current_user_or_guest
  ...
  def show_author_note
    if(@author_note.changeable?(current_user_or_guest))
  ...
  def create
    if (@author_note.changeable?(current_user_or_guest) && @author_note.update_attributes(params[:author_note]))      
