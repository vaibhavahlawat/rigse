class ActivitiesController < ApplicationController
  ...
  def can_edit
    if defined? @activity
      unless @activity.changeable?(current_user_or_guest_or_guest)
        error_message = "you (#{current_user_or_guest_or_guest.login}) can not #{action_name.humanize} #{@activity.name}"
  ...
  def index
    ...
    if params[:mine_only]
      @activities = @activities.reject { |i| i.user.id != current_user_or_guest.id }
    end
  ...
  def duplicate
    ...
    @activity.deep_set_user current_user_or_guest
    @activity.save
